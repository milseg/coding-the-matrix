...
part_3_BA = [[2,0,1,5],[3,0,-4,2],[3,4,0,-2],[1,-4,6,2]]
...
column_row_vector_multiplication2 = Vec({0, 1, 2}, {0:24, 1:9, 2:4})
...
def lin_comb_mat_vec_mult(M, v):
    ...
    ret = Vec(M.D[0], {})
    cd = mat2coldict(M)
    return sum([v[k]*c for k,c in cd.items()])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    ret = Vec(M.D[1], {})
    rd = mat2rowdict(M)
    return sum([v[k]*r for k,r in rd.items()])
...
def dot_product_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    rd = mat2rowdict(M)
    return Vec(M.D[0], {k: r*v for k,r in rd.items()})
...
def dot_product_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    cd = mat2coldict(M)
    return Vec(M.D[1], {k: v*c for k,c in cd.items()})
